...
class LoggerClassGeneratorAstValidator {
  ...
  static void validateLoggerImports(
    String generatedCode,
    ...
    expect(
      AstHelper.hasImport(unit, 'package:flutter/foundation.dart'),
      isTrue,
      reason: 'Should import Flutter foundation',
    );
    expect(
      AstHelper.hasImport(unit, 'package:logger/logger.dart'),
      isTrue,
      reason: 'Should import logger package',
    );

    // Custom imports from config
    for (final importPath in expectedImports) {
      expect(
        AstHelper.hasImport(unit, importPath),
        isTrue,
        reason: 'Should import $importPath',
      );
    }
  }
...
class LoggerComponentAstValidator {
  ...
  static void validateLoggerFactory(
    ...
    if (factoryFunction != null) {
      ...
      _validateFactoryOutputs(
        factoryFunction,
        expectedOutputs,
        disableReleaseConsoleOutput,
      );
    }
  }
  ...
  static void _validateLogMethod(ClassDeclaration printerClass) {
    ...
    if (logMethod != null) {
      ...
      expect(
        AstHelper.hasOverrideAnnotation(logMethod),
        isTrue,
  ...
  static void _validatePrinterHelperMethods(ClassDeclaration printerClass) {
    final helperMethods = [
      ...
      '_findMostMatchedTrace',
      '_doesTraceContainsAllKeywords',
    ];
  ...
  static void _validateFactoryParameters(FunctionDeclaration factoryFunction) {
    final parameters =
        factoryFunction.functionExpression.parameters?.parameters ?? [];
  ...
  static VariableDeclarationList? _findTopLevelVariable(
